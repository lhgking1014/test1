...
def _load_news_payload(
    force_refresh: bool = False, excluded_keys: Iterable[str] | None = None
) -> Dict:
    """Return cached Tesla news payload, refreshing if requested or missing."""
    ensure_data_directory()
    if force_refresh:
        return write_news(collect_news(exclude_keys=excluded_keys))
    excluded_set = set(filter(None, excluded_keys or []))
    if DATA_PATH.exists():
        try:
            ...
            if excluded_set:
                payload["items"] = [
                    ...
                    if (item.get("url") or item.get("title")) not in excluded_set
                ]
            return payload
        except json.JSONDecodeError:
            st.warning("저장된 뉴스 데이터가 손상되어 새로 불러옵니다.")
    payload = write_news(collect_news(exclude_keys=excluded_set))
    if excluded_set:
        payload["items"] = [
            item
            for item in payload.get("items", [])
...
def _collect_low_relevance_keys(evaluations: Dict) -> List[str]:
    keys: List[str] = []
    seen = set()
...
def _reset_rating_state() -> None:
    for key in list(st.session_state.keys()):
        if key.startswith("rating_value_") or key.startswith("reason_") or key.startswith("radio_"):
...
def refresh_news() -> None:
    with st.spinner("최신 뉴스를 불러오는 중..."):
        evaluations = st.session_state.get("evaluations_payload", {})
        excluded = _collect_low_relevance_keys(evaluations)
        st.session_state["news_payload"] = _load_news_payload(
            force_refresh=True,
        ...
        with container:
            ...
            if rating_value == "low":
                ...
                st.text_area(
                    ...
                    key=reason_key,
                    placeholder="요약이 부족하거나 주제가 맞지 않는 이유를 적어주세요.",
                )
            else:
